...
// ==========================================
// [MÓDULO 01] - CONFIGURACIÓN Y CONSTANTES GLOBALES
// ==========================================
const COSTO_EMPAQUE_FIJO = 1000;
...
const ADMIN_UID_BYPASS = "ADMIN_MASTER";
...
// ==========================================
// [MÓDULO 02] - INTERFAZ UI Y TRIGGERS
// ==========================================
function onOpen() {
  ...
  ui.createMenu('🍔 HUNGER BURGERS')
      ...
      .addSeparator()
      .addItem('🔓 LIMPIAR BLACKLIST (DESBLOQUEAR)', 'limpiarBlacklist')
      .addSeparator()
...
function clearRecetasCache() { CacheService.getScriptCache().remove("RECETAS_DATA"); }
...
// ==========================================
// [MÓDULO 04] - SEGURIDAD Y BLACKLIST
// ==========================================
function verificarBloqueo(uid, celular) {
  if (uid === ADMIN_UID_BYPASS) return false;
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (baneado === String(uid).trim() || (celular && baneado === String(celular).trim())) return true;
  }
  return false;
}

function registrarEnBlacklist(identificador, motivo) {
  if (!identificador || identificador === ADMIN_UID_BYPASS) return;
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let sh = ss.getSheetByName("BLACKLIST");
  if (!sh) {
...
function limpiarBlacklist() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sh = ss.getSheetByName("BLACKLIST");
  if (sh) {
    sh.clearContents();
    sh.appendRow(["IDENTIFICADOR", "MOTIVO", "FECHA"]);
    sh.getRange("A1:C1").setBackground("#000000").setFontColor("white").setFontWeight("bold");
    SpreadsheetApp.getUi().alert("✅ Blacklist purgada con éxito. Accesos restaurados.");
  } else {
    SpreadsheetApp.getUi().alert("⚠️ No existe la hoja BLACKLIST.");
  }
}

// ==========================================
// [MÓDULO 05] - NÚCLEO DE INVENTARIO Y COSTOS
// ==========================================
function motorInventario(ss, prod, cant, modoA, omitir = [], cacheHojasObj = {}, cacheDataObj = {}, tipoPedido = "DOMICILIO", hojasModificadas = new Set()) {
  ...
  for (let i = 1; i < recetas.length; i++) {
    ...
    if (itemRec === nombreProdLimpio) {
      ...
      let hojaDestino = recetas[i][3] ? String(recetas[i][3]).trim() : "N/A", ingredienteOriginal = recetas[i][1] ? String(recetas[i][1]).trim() : "", cantIngrediente = Number(recetas[i][2]) || 0;
      if (hojaDestino === "N/A" || !hojaDestino) continue;
      ...
      if (hojaDestino.toUpperCase() === "RECETAS") { motorInventario(ss, ingredienteOriginal, cantIngrediente * Number(cant), modoA, omitir, cacheHojasObj, cacheDataObj, tipoPedido, hojasModificadas); continue; }
      
      if (!cacheHojasObj[hojaDestino]) { cacheHojasObj[hojaDestino] = ss.getSheetByName(hojaDestino);
        if (cacheHojasObj[hojaDestino]) cacheDataObj[hojaDestino] = cacheHojasObj[hojaDestino].getDataRange().getValues(); }
      const shI = cacheHojasObj[hojaDestino], dI = cacheDataObj[hojaDestino];
      if (shI && dI) {
        for (let j = 1; j < dI.length; j++) {
          if ((dI[j][0] ? String(dI[j][0]).trim().toUpperCase() : "") === ingredienteLimpio) {
            let rendimiento = Number(dI[j][8]) || 1, gastoPorcion = (1 / rendimiento) * cantIngrediente * Number(cant);
            let salidasActuales = Number(dI[j][3]) || 0, nuevasSalidas = modoA ? salidasActuales - gastoPorcion : salidasActuales + gastoPorcion;
            if (nuevasSalidas < 0) nuevasSalidas = 0;
            let stockInicial = Number(dI[j][1]) || 0, entradas = Number(dI[j][2]) || 0;
            dI[j][3] = nuevasSalidas; dI[j][4] = stockInicial + entradas - nuevasSalidas;
            hojasModificadas.add(hojaDestino); break;
          }
        }
      }
    }
  ...
  if (!encontradoEnRecetas) {
    ...
    for (let h of hojasInv) {
      if (!cacheHojasObj[h]) { cacheHojasObj[h] = ss.getSheetByName(h);
        if (cacheHojasObj[h]) cacheDataObj[h] = cacheHojasObj[h].getDataRange().getValues(); }
      const shI = cacheHojasObj[h], dI = cacheDataObj[h];
      if (!shI || !dI) continue;
      for (let j = 1; j < dI.length; j++) {
        if ((dI[j][0] ? String(dI[j][0]).trim().toUpperCase() : "") === nombreProdLimpio) {
          let salidasActuales = Number(dI[j][3]) || 0, nuevasSalidas = modoA ? salidasActuales - cant : salidasActuales + cant;
          if (nuevasSalidas < 0) nuevasSalidas = 0;
          dI[j][3] = nuevasSalidas; dI[j][4] = (Number(dI[j][1]) || 0) + (Number(dI[j][2]) || 0) - nuevasSalidas;
          hojasModificadas.add(h); return;
        }
      }
    }
  }
...
function acumularRequerimientos(nombreProd, cant, cacheHojas, reqMap, tipoPedido = "DOMICILIO") {
  ...
  for (let i = 1; i < rec.length; i++) {
    ...
    if (itemRec === nombreTrim) {
      encontradoEnRecetas = true;
      let hojaDestino = rec[i][3] ? String(rec[i][3]).trim() : "N/A", ingredienteOriginal = rec[i][1] ? String(rec[i][1]).trim() : "", cantIngrediente = Number(rec[i][2]) || 0;
      if (hojaDestino === "N/A" || !hojaDestino) continue;
      let esParaLlevar = /\[LLEVAR\]/i.test(ingredienteOriginal);
      if (esParaLlevar && tipoPedido === "LOCAL") continue;
      let ingredienteLimpio = ingredienteOriginal.replace(/\[LLEVAR\]/ig, "").trim().toUpperCase();
...
function obtenerCostoIngrediente(ss, hoja, ing, cantR, cacheData = null, tipoPedido = "DOMICILIO") {
  ...
  let hojaLimpia = String(hoja).trim().toUpperCase(), nombreIngRaw = String(ing).trim();
  ...
  if (hojaLimpia === "RECETAS") {
    ...
    for (let i = 1; i < cacheData["RECETAS"].length; i++) {
      if ((cacheData["RECETAS"][i][0] ? String(cacheData["RECETAS"][i][0]).trim().toUpperCase() : "") === nombreIng) costoSubReceta += obtenerCostoIngrediente(ss, cacheData["RECETAS"][i][3], cacheData["RECETAS"][i][1], Number(cacheData["RECETAS"][i][2]), cacheData, tipoPedido);
    }
    return costoSubReceta * Number(cantR);
  }
  
  if (!cacheData[hojaLimpia]) { let sh = ss.getSheetByName(hojaLimpia);
    cacheData[hojaLimpia] = sh ? sh.getDataRange().getValues() : []; }
  for (let i = 1; i < cacheData[hojaLimpia].length; i++) {
    if ((cacheData[hojaLimpia][i][0] ? String(cacheData[hojaLimpia][i][0]).trim().toUpperCase() : "") === nombreIng) return (Number(cacheData[hojaLimpia][i][6]) / (Number(cacheData[hojaLimpia][i][8]) || 1)) * Number(cantR);
  }
  return 0;
}
...
// ==========================================
// [MÓDULO 06] - GESTIÓN DE PEDIDOS Y RUTAS
// ==========================================
function calcularTarifa(direccionDestino) {
  ...
  try {
    ...
    const direcciones = Maps.newDirectionFinder().setOrigin(ORIGEN_RESTAURANTE).setDestination(destinoFull).setMode(Maps.DirectionFinder.Mode.DRIVING).getDirections();
    if (direcciones.status !== 'OK' || !direcciones.routes[0]) return { costo: VALOR_BASE_DOMICILIO, km: 0, error: "Ubicación ambigua. Tarifa base aplicada preventivamente." };
    const leg = direcciones.routes[0].legs[0];
    const distanceKm = leg.distance.value / 1000;
    if (distanceKm > RADIO_MAXIMO_KM) return { costo: 0, km: distanceKm, error: `Fuera de zona. Máximo ${RADIO_MAXIMO_KM} km.` };
    let tarifaFinal = VALOR_BASE_DOMICILIO;
    if (distanceKm > 1) tarifaFinal += (Math.ceil(distanceKm - 1) * VALOR_KM_ADICIONAL);
    return { costo: tarifaFinal, km: distanceKm.toFixed(1), error: null };
  } catch (err) { return { costo: VALOR_BASE_DOMICILIO, km: 0, error: "Error de red. Tarifa base aplicada." }; }
}
...
function guardarPedidoPOS(clienteObj, carritoJSON, uid) {
  ...
  try {
    ...
    if (tipo_pedido === "DOMICILIO" && !carrito.find(i => String(i.nombre).toUpperCase() === "DOMICILIO")) carrito.push({nombre: "DOMICILIO", cant: 1, precioTotalCalculado: VALOR_BASE_DOMICILIO});
    if ((tipo_pedido === "DOMICILIO" || tipo_pedido === "PARA LLEVAR") && !carrito.find(i => String(i.nombre).toUpperCase() === "COSTO EMPAQUE")) carrito.push({nombre: "COSTO EMPAQUE", cant: 1, precioTotalCalculado: COSTO_EMPAQUE_FIJO});
    
    const recetasData = getRecetasCached();
    let preciosSeguros = {};
    for(let r = 1; r < recetasData.length; r++) if(recetasData[r][0]) preciosSeguros[String(recetasData[r][0]).trim().toUpperCase()] = Number(recetasData[r][4]) || 0;
    ...
    if (carrito.length === 0) {
       ...
       carrito.forEach(item => {
           ...
           let esBebidaComercial = ["DEL VALLE", "COCACOLA", "SPRITE", "PREMIO", "CUATRO", "JUGO", "LIMONADA", "AGUA"].some(b => nombreItemTemp.includes(b));
           let esPrincipal = !nombreItemTemp.includes("SALSA") && !nombreItemTemp.includes("DOMICILIO") && !nombreItemTemp.includes("EMPAQUE") && nombreItemTemp !== "CABRA DE ORO" && !nombreItemTemp.includes("GUACAMOLE") && !nombreItemTemp.includes("CHIMICHURRI") && !nombreItemTemp.includes("TÁRTARA") && !esBebidaComercial;
           if (esPrincipal) {
               let cantP = Math.max(1, Number(item.cant) || 1); 
               salsasGratisDisponibles += (cantP * 3); 
               platosPrincipales += cantP;
           }
       });
       ...
       let filas = carrito.map(item => {
          ...
          if (nombreItem === "DOMICILIO") {
              ...
              let cantEmpaques = platosPrincipales > 0 ? platosPrincipales : 1;
              precioForzado = COSTO_EMPAQUE_FIJO * cantEmpaques;
              if (precioFrontend < precioForzado) precioFrontend = precioForzado;
              cantReal = cantEmpaques;
          } else if (nombreItem.includes("SALSA") || nombreItem.includes("GUACAMOLE") || nombreItem.includes("CHIMICHURRI") || nombreItem.includes("TÁRTARA")) {
              let salsasACobrar = 0;
              if (salsasGratisDisponibles >= cantReal) { salsasGratisDisponibles -= cantReal; salsasACobrar = 0; } 
              else { salsasACobrar = cantReal - salsasGratisDisponibles; salsasGratisDisponibles = 0; }
              precioForzado = precioBaseBd * salsasACobrar;
              if (uid !== ADMIN_UID_BYPASS && precioFrontend < precioForzado) precioFrontend = precioForzado; 
          } else if (nombreItem !== "CABRA DE ORO") {
              if (uid !== ADMIN_UID_BYPASS && precioFrontend < precioForzado) manipulacionDetectada = true;
          }
       ...
       if (manipulacionDetectada && uid !== ADMIN_UID_BYPASS) {
    ...
    return idOficial;
      ...
      if (e.message.includes("ACCESO DENEGADO") || e.message.includes("TRANSACCIÓN ABORTADA")) throw e;
      throw new Error("El sistema de colas está procesando demasiados pedidos. Por favor, reintenta en un momento.");
  } finally {
      lock.releaseLock();
  }
}
...
function modificarPedidoPOS(idAEditar, clienteObj, carritoJSON) {
  ...
  try {
    ...
    let filasBorradas = false, hojasModificadas = new Set(), cacheHojasObj = {}, cacheDataObj = {};
    for (let i = d.length - 1; i >= 1; i--) {
      let idActual = d[i][0] ? String(d[i][0]).trim() : "";
      if (idActual === String(idAEditar).trim()) {
         let estado = d[i][3] ? String(d[i][3]).trim() : "";
         if (estado !== "PENDIENTE" && estado !== "POR PAGAR 💰" && estado !== "EN COCINA 👨‍🍳") throw new Error("Pedido en reparto o despachado. No modificable.");
         if (estado === "EN COCINA 👨‍🍳") {
    ...
    for (let hoja of hojasModificadas) {
      let sheet = cacheHojasObj[hoja], data = cacheDataObj[hoja];
      sheet.getRange(1, 1, data.length, data[0].length).setValues(data);
    }

    if (!filasBorradas) throw new Error("No se encontró el pedido a modificar.");
    return guardarPedidoPOS(clienteObj, carritoJSON, ADMIN_UID_BYPASS);
  } finally { lock.releaseLock(); }
}

function guardarExtraTicketRemoto(idPedido, extraName, cobroExtra) {
  const ss = SpreadsheetApp.getActiveSpreadsheet(), shP = ss.getSheetByName("PEDIDOS_ACTIVOS"), d = shP.getDataRange().getValues();
  let datosPedido = null;
  for (let i = 1; i < d.length; i++) if (String(d[i][0]).trim() === String(idPedido).trim()) { datosPedido = d[i]; break; }
  if (!datosPedido) throw new Error("Pedido no encontrado.");
  shP.appendRow([String(idPedido).trim(), "EXTRA: " + String(extraName).toUpperCase(), 1, datosPedido[3], new Date(), datosPedido[5], datosPedido[6], "Agregado KDS", Number(cobroExtra) || 0, datosPedido[9], datosPedido[10], 0, datosPedido[12]]);
  return "OK";
}
...
// ==========================================
// [MÓDULO 07] - KITCHEN DISPLAY SYSTEM (KDS) Y POS
// ==========================================
function buscarEstadoPedido(turnoBuscado) {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const shP = ss.getSheetByName("PEDIDOS_ACTIVOS");
  const d = shP.getDataRange().getValues();
  let idOficial = "", cliente = "Cliente", estado = "PENDIENTE", tipo = "LOCAL", metodo = "EFECTIVO", totalCalculado = 0, encontrado = false, items = [];
  for (let i = d.length - 1; i >= 1; i--) { 
    ...
    if (idCompleto && (idCompleto.includes("-" + turnoBuscado + "-") || idCompleto.endsWith("-" + turnoBuscado) || idCompleto === turnoBuscado)) {
      ...
      cliente = d[i][5] || "Cliente"; estado = d[i][3] || "PENDIENTE"; tipo = d[i][9] || "LOCAL"; metodo = d[i][10] || "Efectivo";
  ...
  for (let i = 1; i < d.length; i++) {
    ...
    if (idCompleto === idOficial) {
      ...
      if (nombreItem && nombreItem !== "ORDEN VACÍA") items.push({ nombre: nombreItem, cant: cantItem });
    }
  }
...
function obtenerMenuPOS() {
  ...
  let mapPOS = {}, reqEmpaque = {}, catMap = {};
  for (let i = 1; i < data.length; i++) {
    let prod = String(data[i][0]).trim().toUpperCase(), ing = String(data[i][1]).trim().toUpperCase(), precio = Number(data[i][4]) || 0;
    let categoriaRaw = String(data[i][5] || "").trim().toUpperCase(), imgUrl = String(data[i][6] || "").trim(), descripcionTexto = String(data[i][7] || "").trim(); 
    if (!prod) continue;
    if (categoriaRaw !== "" && !catMap[prod]) catMap[prod] = categoriaRaw;
    
    if (precio > 0) {
        if (mapPOS[prod] === undefined || mapPOS[prod].precio === 0) mapPOS[prod] = { precio: precio, imagen: imgUrl, descripcion: descripcionTexto };
        else if (descripcionTexto !== "" && !mapPOS[prod].descripcion) mapPOS[prod].descripcion = descripcionTexto;
    }
  ...
  for (let prod in mapPOS) {
    ...
    catalogo.push({ nombre: prod, precio: mapPOS[prod].precio, imagen: mapPOS[prod].imagen, requiereEmpaque: !!reqEmpaque[prod], categoria: catOficial, descripcion: mapPOS[prod].descripcion || "" });
  }
...
function obtenerPedidosKDS() {
  ...
  let precios = {}, reqEmpaque = {}, categorias = {};
  for (let r = 1; r < rec.length; r++) {
    ...
    let cat = rec[r][5] ? String(rec[r][5]).trim().toUpperCase() : "";
  ...
  for (let i = 0; i < d.length; i++) {
    ...
    let fechaCreacion = d[i][4] ? new Date(d[i][4]).getTime() : new Date().getTime();

    if (est === "PENDIENTE" || est === "EN COCINA 👨‍🍳" || est === "POR PAGAR 💰") {
      if (!ticketsMap[id]) ticketsMap[id] = { id: id, cliente: d[i][5], celular: d[i][6], direccion: d[i][12], tipo: d[i][9], notas: d[i][7], est: est, items: [], itemsObj: [], total: 0, metodo_pago: d[i][10] || "Efectivo", bloqueado: false, motivosBloqueo: [], timestamp: fechaCreacion };
      ...
      let catItem = categorias[nombreProd] || "";
      let isSub = false;
      if (catItem === "SALSA" || catItem === "ADICIÓN" || catItem === "ADICION" || catItem === "EXTRA" || nombreProd.startsWith("EXTRA:") || nombreProd.includes("SALSA") || nombreProd === "GUACAMOLE ESPECIAL" || nombreProd === "CHIMICHURRI") {
          if (!nombreProd.includes("PAPAS")) {
              isSub = true;
          }
      }
      ...
      ticketsMap[id].itemsObj.push({ nombre: nombreProd, cant: cantItem, precio: precioUnitario, isSubItem: isSub });
...
// ==========================================
// [MÓDULO 08] - FLUJO DE TRABAJO (ESTADOS DE PEDIDO)
// ==========================================
function avanzarTicketCompleto(idPedido, omitirStr = "") {
  try {
      ...
      const d = shP.getDataRange().getValues();
      const rec = getRecetasCached();
      let clienteActualizado = false, cacheHojas = {}, cacheData = { "RECETAS": rec }, hojasModificadas = new Set(), tipoPedidoTicket = "LOCAL";
      for (let i = 1; i < d.length; i++) {
        if ((d[i][0] ? String(d[i][0]).trim() : "") === idBuscado) { tipoPedidoTicket = d[i][9] ? String(d[i][9]).trim().toUpperCase() : "LOCAL"; break; }
      }
      ...
      for (let i = 1; i < d.length; i++) {
        if ((d[i][0] ? String(d[i][0]).trim() : "") === idBuscado) {
          let est = d[i][3] ? String(d[i][3]).trim() : "", prodActual = d[i][1] ? String(d[i][1]).trim().toUpperCase() : "";
          if (est === "PENDIENTE") {
            let pVenta = 0, cTotal = 0;
            for (let r = 1; r < rec.length; r++) {
              if ((rec[r][0] ? String(rec[r][0]).trim().toUpperCase() : "") === prodActual) {
            ...
            let celdaPrecio = d[i][8], pVentaFinal = (celdaPrecio !== "" && celdaPrecio !== null) ? (Number(celdaPrecio) || 0) / Number(d[i][2]) : pVenta;
      ...
      for (let hoja of hojasModificadas) {
        let sheet = cacheHojas[hoja], data = cacheData[hoja];
        sheet.getRange(1, 1, data.length, data[0].length).setValues(data);
        for (let j = 1; j < data.length; j++) {
            let stockActual = Number(data[j][4]) || 0, stockMinimo = Number(data[j][7]) || 0;
...
// ==========================================
// [MÓDULO 09] - FINANZAS, CIERRE Y CLIENTES
// ==========================================
...
function cierreDeTurno() {
  ...
  try {
    ...
    const d = shP.getDataRange().getValues();
    let v = 0, tD = 0, uT = 0, anuladosCount = 0, valorAnulados = 0, tComisionKernel = 0, tNequi = 0, tEfectivo = 0;
    let newData = [d[0]], historicoData = [], ticketsUnicos = new Set(), ticketsAnuladosUnicos = new Set();
    for (let i = 1; i < d.length; i++) {
      let estado = String(d[i][3]).trim(), idPedido = String(d[i][0]).trim(), precioItem = Number(d[i][8]) || 0, utilidadItem = Number(d[i][11]) || 0;
      let metodo = d[i][10] ? String(d[i][10]).trim().toUpperCase() : "EFECTIVO", nombreProd = String(d[i][1]).trim().toUpperCase();
      if (estado === "ENTREGADO ✅") { 
        v++; ticketsUnicos.add(idPedido); tD += precioItem;
        uT += utilidadItem;
        if (nombreProd !== "DOMICILIO" && nombreProd !== "COSTO EMPAQUE") tComisionKernel += (precioItem * TASA_KERNEL);
        if (metodo.includes("NEQUI")) tNequi += precioItem; else tEfectivo += precioItem; 
        historicoData.push(d[i]);
      } else if (estado === "❌ ANULADO") { 
        anuladosCount++; ticketsAnuladosUnicos.add(idPedido);
        valorAnulados += precioItem; historicoData.push(d[i]);
      } else { newData.push(d[i]); }
    }

    let numPedidosReales = ticketsUnicos.size, ticketPromedio = numPedidosReales > 0 ? (tD / numPedidosReales) : 0, rentabilidad = tD > 0 ? (uT / tD) : 0;
    if (v > 0) {
        shB.appendRow([new Date(), tD, v, numPedidosReales, anuladosCount, "Cierre Exitoso", uT, ticketPromedio, rentabilidad, tNequi, tEfectivo, valorAnulados, tComisionKernel]);
        let lastRow = shB.getLastRow();
        shB.getRange(lastRow, 8).setNumberFormat("$#,##0"); shB.getRange(lastRow, 9).setNumberFormat("0.00%"); shB.getRange(lastRow, 10).setNumberFormat("$#,##0"); 
        shB.getRange(lastRow, 11).setNumberFormat("$#,##0"); shB.getRange(lastRow, 12).setNumberFormat("$#,##0"); shB.getRange(lastRow, 13).setNumberFormat("$#,##0");
    }

    if (historicoData.length > 0) shH.getRange(shH.getLastRow() + 1, 1, historicoData.length, historicoData[0].length).setValues(historicoData);
...
// ==========================================
// [MÓDULO 10] - COMPRAS, MERMAS Y PROMOCIONES
// ==========================================
...
function obtenerConfigPromos() {
  const props = PropertiesService.getScriptProperties();
  return {
    subastaActiva: props.getProperty('PROMO_SUBASTA') !== 'false', 
    cabraActiva: props.getProperty('PROMO_CABRA') !== 'false',     
    estampidaGlobal: Number(props.getProperty('CABRA_GLOBAL_TS') || 0) > Date.now()
  };
}

function guardarConfigPromos(subasta, cabra) {
  const props = PropertiesService.getScriptProperties();
  props.setProperty('PROMO_SUBASTA', subasta);
  props.setProperty('PROMO_CABRA', cabra);
  return "Configuración guardada";
}

function lanzarEstampidaGlobal() {
  const props = PropertiesService.getScriptProperties();
  props.setProperty('CABRA_GLOBAL_TS', Date.now() + 300000);
  return "🐐 ¡Estampida lanzada! Todos los clientes verán la cabra en el próximo minuto.";
}
